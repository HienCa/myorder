...
class SpecificationsController extends GetxController {
  getVatById(String vat_id) async {
    try {
      DocumentSnapshot Vat =
          await firestore.collection('specifications').doc(vat_id).get();
      if (Vat.exists) {
  ...
  getVats(String keySearch) async {
    if (keySearch.isEmpty) {
      _vats.bindStream(
        firestore.collection('specifications').snapshots().map(
          (QuerySnapshot query) {
            List<Vat> retValue = [];
    ...
    try {
      if (name.isNotEmpty) {
        ...
        CollectionReference usersCollection =
            FirebaseFirestore.instance.collection('specifications');
  ...
  updateVat(
    ...
    try {
      await firestore.collection('specifications').doc(vat_id).update({
        "name": name.trim().toUpperCase(),
        "vat_percent": int.tryParse(vat_percent) ?? 0,
      });
